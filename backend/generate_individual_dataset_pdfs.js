/**
 * PDF Generator for ALL 14 Individual Government Datasets + Master Combined PDF
 */
...
const outputDir = path.resolve(__dirname, '../Govt_Data_PDFs');
fs.mkdirSync(outputDir, { recursive: true });

const artifactDir = path.resolve('C:/Users/V R SUHAAS GOWDA/.gemini/antigravity-ide/brain/ae684536-666d-41a3-b67d-7ee695bdcb26/Govt_Data_PDFs');
fs.mkdirSync(artifactDir, { recursive: true });

// Universal Single-Page PDF Table Generator
function generateSingleDatasetPDF({ filename, title, authority, identifierLabel, dataset, columns, colWidths }) {
  ...
  try {
    const writeStream2 = fs.createWriteStream(artifactFilePath);
  ...
  doc.fillColor('#f0f4ff').fontSize(10).font('Helvetica-Bold').text(title, 30, 24, { lineBreak: false });
  doc.fillColor('#94a3b8').fontSize(7).font('Helvetica').text(`Authority: ${authority}  •  Primary Key: ${identifierLabel}  •  20 Records Synchronized`, 30, 38, { lineBreak: false });

  // Badge
  doc.rect(640, 10, 172, 34).fill('#1e293b');
  doc.fillColor('#10b981').fontSize(7.5).font('Helvetica-Bold').text('✓ SYNTHETIC GOVERNMENT DATASET', 648, 16, { lineBreak: false });
  doc.fillColor('#94a3b8').fontSize(6.5).font('Helvetica').text('SIH 2026 Procurement Prototype', 648, 28, { lineBreak: false });
  ...
  columns.forEach((col, i) => {
    doc.text(col.label, colXs[i] + 3, startY + 5, { width: colWidths[i] - 6, align: col.align || 'left', lineBreak: false });
  });
  ...
  doc.fillColor('#94a3b8').fontSize(7).font('Helvetica')
    .text(`ComplyGeM AI Verification Platform  •  SIH 2026  •  ${title}`, 30, 578, { lineBreak: false });
  doc.fillColor('#38bdf8').fontSize(7).font('Helvetica-Bold')
    .text('Page 1 of 1  •  Complete Dataset PDF', 600, 578, { width: 210, align: 'right', lineBreak: false });
...
// 1. PAN Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '01_CBDT_PAN_Dataset.pdf',
  title: 'Income Tax Department (CBDT) — Permanent Account Number (PAN) Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '02_GSTN_Verification_Dataset.pdf',
  title: 'Goods & Services Tax Network (GSTN) — Taxpayer & Filing Regularity Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '03_MCA21_Corporate_Registry_Dataset.pdf',
  title: 'Ministry of Corporate Affairs (MCA21) — Corporate Identity (CIN/LLPIN) Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. Udyam MSME Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '04_Udyam_MSME_Registry_Dataset.pdf',
  title: 'Ministry of MSME — Udyam Registration & Enterprise Classification Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. Income Tax ITR Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '05_Income_Tax_ITR_Dataset.pdf',
  title: 'Income Tax Return (ITR) Compliance & CPC e-Filing Gateway',
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. EPFO Labour Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '06_EPFO_Labour_Compliance_Dataset.pdf',
  title: "Employees' Provident Fund Organisation (EPFO) — Shram Suvidha Portal",
...
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '07_ESIC_Labour_Welfare_Dataset.pdf',
  title: "Employees' State Insurance Corporation (ESIC) — Statutory Insurance Registry",
  authority: 'Ministry of Labour and Employment',
...
// ─────────────────────────────────────────────────────────────────────────────
// 8. DigiLocker Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '08_DigiLocker_Verified_Documents_Dataset.pdf',
  title: 'DigiLocker Gateway — Cryptographically Signed Verified Credentials Repository',
...
// ─────────────────────────────────────────────────────────────────────────────
// 9. Startup India Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '09_DPIIT_Startup_India_Dataset.pdf',
  title: 'Startup India — DPIIT Recognition, Tax & Tender Relaxation Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
// 10. NSIC Single Point Registration Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '10_NSIC_Single_Point_Registration_Dataset.pdf',
  title: 'National Small Industries Corporation (NSIC) — Single Point Registration Registry',
  authority: 'Ministry of Micro, Small and Medium Enterprises',
...
// ─────────────────────────────────────────────────────────────────────────────
// 11. GeM Seller Registry Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '11_GeM_Seller_Registry_Dataset.pdf',
  title: 'Government e-Marketplace (GeM) — Seller Account & Performance Registry',
...
// ─────────────────────────────────────────────────────────────────────────────
// 12. BIS Standards Quality Certifications Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '12_BIS_Quality_Certifications_Dataset.pdf',
  title: 'Bureau of Indian Standards (BIS Manak Online) — Quality & Conformity Certifications',
...
// ─────────────────────────────────────────────────────────────────────────────
// 13. Make in India Local Content Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '13_Make_In_India_Local_Content_Dataset.pdf',
  title: 'Make in India (MII) & Public Procurement Preference — Local Content % Declarations',
...
// ─────────────────────────────────────────────────────────────────────────────
// 14. Central Debarment & Blacklist Dataset PDF
// ─────────────────────────────────────────────────────────────────────────────
generateSingleDatasetPDF({
  filename: '14_Central_Debarment_Blacklist_Dataset.pdf',
  title: 'Central Vigilance Commission & National Debarment / Blacklist Registry',
...
console.log('All 14 Individual Datasets generated successfully!');
